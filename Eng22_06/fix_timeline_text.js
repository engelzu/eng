...
files.forEach(f => {
  ...
  // Replace Title
  code = code.replace('"CRONOGRAMA FÍSICO FINANCEIRO"', '"CRONOGRAMA EAP"');
  code = code.replace('"Visualização do cronograma físico-financeiro do projeto"', '"Visualização do cronograma EAP do projeto"');

  // We need to replace the timeline rendering code again to update the labels and font sizes
  ...
  if (code.includes(targetTimeline)) {
    ...
    console.log('Fixed timeline text visibility in', f);
  } else {
