...
saveVars += '...(bcEnviarAprovacao?{status:"AGUARDANDO APROVAÇÃO"}:(bcStatusAnterior?{status:bcStatusAnterior}:{})),';

let applyLogic = "eh();setShowBcModal(!1);";
...
for (const f of files) {
  ...
  if (code.includes(modalTarget) && !code.includes('open:showBcModal')) {
    code = code.replace(modalTarget, '${modalMarkup}' + modalTarget);
